...
var _xNotify = require("./x-notify");
const Notify = new (0, _xNotify.XNotify)("TopRight");
...
const datePicker = (0, _flatpickrDefault.default)(dateField, options);
...
document.querySelectorAll(".field").forEach((elem)=>{
    elem.style.cssText += "text-align: center;font-size: 25px;margin-top: 0px;display: inline-flex;flex-direction: column;";
    elem.lastElementChild.style.cssText += "addLeadingZeroding-top: 5px;font-size: 14px;text-transform: uppercase;";
});
...
function startCountDownHandler() {
    ...
    let timerId = setInterval(function() {
        ...
        dayField.innerHTML = addLeadingZero(Math.floor(difference / 86400000));
        hourField.innerHTML = addLeadingZero(Math.floor(difference % 86400000 / 3600000));
        minuteField.innerHTML = addLeadingZero(Math.floor(difference % 3600000 / 60000));
        secondField.innerHTML = addLeadingZero(Math.floor(difference % 60000 / 1000));
        if (difference < 1000) {
...
},{"flatpickr":"llQu5","flatpickr/dist/flatpickr.min.css":"eVN6V","./x-notify":"6sU1C","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"llQu5":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
},{}],"eVN6V":[function() {},{}],"6sU1C":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "XNotify", ()=>XNotify);
class XNotify {
    ...
    hideNotification(element) {
        ...
        if (this.empty(container.innerHTML)) container.remove();
    }
    clear() {
        let container = document.getElementById("x-notify-container");
        let notifications = container.getElementsByClassName("x-notification");
        for(let i = 0; i < notifications.length; i++)this.hideNotification(notifications[i]);
    }
    generateID() {
        let id = this.epoch() + "-" + this.shuffle(this.epoch());
        if (this.empty(document.getElementById("x-notify-container").innerHTML)) return id;
        let invalid = true;
        while(invalid)if (document.getElementById(id)) id = this.epoch() + "-" + this.shuffle(this.epoch());
        else {
            invalid = false;
            break;
        }
    ...
    empty(value) {
        if (value === null || typeof value === "undefined" || value.toString().trim() === "") return true;
        return false;
    }
}

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["8NMcg","1AWyZ"], "1AWyZ", "parcelRequire7bc7")
